...
def get_admin_deal_actions_keyboard(deal_id: int, status: str = "") -> InlineKeyboardMarkup:
    kb = []
    if status in [DealStatus.UNPAID, DealStatus.PENDING_PARTNER, DealStatus.DISPUTED, ""]:
        kb.append([
            InlineKeyboardButton(text="🔒 Подтвердить оплату (В гарант)", callback_data=f"adm_deal_hold:{deal_id}")
        ])

    kb.append([
        InlineKeyboardButton(text="✅ Завершить и выплатить продавцу", callback_data=f"adm_deal_complete:{deal_id}"),
    ])
    kb.append([
